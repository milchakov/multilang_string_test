...
namespace
{
  ...
  OldStringUtf8Multilang::Languages const g_languages = {{ {"default", "Native for each country"},
...
// static
OldStringUtf8Multilang::Languages const & OldStringUtf8Multilang::GetSupportedLanguages()
{
  // Asserts for generic class constants.
  return g_languages;
}
// static
int8_t OldStringUtf8Multilang::GetLangIndex(string const & lang)
{
...
// static
char const * OldStringUtf8Multilang::GetLangByCode(int8_t langCode)
{
...
// static
char const * OldStringUtf8Multilang::GetLangNameByCode(int8_t langCode)
{
  if (langCode < 0 || langCode > g_languages.size() - 1)
    return "";
...
size_t OldStringUtf8Multilang::GetNextIndex(size_t i) const
{
  ++i;
...
void OldStringUtf8Multilang::AddString(int8_t lang, string const & utf8s)
{
  size_t i = 0;
  size_t const sz = m_s.size();
...
bool OldStringUtf8Multilang::GetString(int8_t lang, string & utf8s) const
{
  size_t i = 0;
  size_t const sz = m_s.size();
...
namespace
{
  ...
  struct Printer
  {
    ...
    {
      m_out += string(OldStringUtf8Multilang::GetLangByCode(code)) + string(":") + name + " ";
      return true;
    }
  };
...
int8_t OldStringUtf8Multilang::FindString(string const & utf8s) const
{
  Finder finder(utf8s);
  ForEach(finder);
  return finder.m_res;
